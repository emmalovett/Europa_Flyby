...
PRO Keck_Europa_Flyby_ALL_DATA, part = part, dir = dir, filt = filt, slit = slit
  common sunlight_fit_common, fitindices

  case dir of
    'Z:\DATA\Keck\Europa Na\HIRES_20220928': begin
      if filt eq 'Na' AND slit eq 'C3' then begin
        ...
        Europa_frames   = string(indgen(7)+120, format='(I4.4)')                         ; K data with C3 slit deck
        Europa_frames   = [Europa_frames, string(indgen(35)+130, format='(I4.4)')]       ; Na data with C3 slit deck, including Juno flyby time
        Europa_frames   = [Europa_frames, string(indgen(25)+165, format='(I4.4)')]       ; K data with C2 slit deck
        Europa_frames   = [Europa_frames, string(indgen(4 )+190, format='(I4.4)')]       ; K data with C3 slit deck
        Jupiter_frames  = string(113, format='(I4.4)')                                   ; Jupiter disk center post eclipse
      endif

      if filt eq 'gg475' AND slit eq 'C2' then begin
        ...
        Europa_frames   = [Europa_frames, string(indgen(25)+165, format='(I4.4)')]       ; K data with C2 slit deck
        Europa_frames   = [Europa_frames, string(indgen(4 )+190, format='(I4.4)')]       ; K data with C3 slit deck
        Jupiter_frames  = string(116, format='(I4.4)')
      endif
      
      if filt eq 'gg475' AND slit eq 'C3' then begin
        biases          = string(indgen(10)+4, format='(I4.4)')
        Flats           = string(indgen(10)+15, format='(I4.4)')
        Lamps           = string(indgen(5)+25, format='(I4.4)')
        Star_Frames     = string(119, format='(I4.4)')
        Europa_frames   = string(indgen(7)+120, format='(I4.4)')                         ; K data with C3 slit deck
        Europa_frames   = [Europa_frames, string(indgen(35)+130, format='(I4.4)')]       ; Na data with C3 slit deck, including Juno flyby time
        Europa_frames   = [Europa_frames, string(indgen(25)+165, format='(I4.4)')]       ; K data with C2 slit deck
        Europa_frames   = [Europa_frames, string(indgen(4 )+190, format='(I4.4)')]       ; K data with C3 slit deck
        Jupiter_frames  = string(115, format='(I4.4)')
      endif
      
      labels = strarr(N_elements(Europa_frames))
      labels[0    ] = 'EW10 N'
      labels[1    ] = 'EW20 N'
      labels[2:4  ] = 'EW on disk'
      labels[5    ] = 'EW10 S'
      labels[6    ] = 'EW20 S'
      labels[7:9  ] = 'EW on disk'
      labels[10:11] = 'EW10 N'
      labels[12:13] = 'EW20 N'
      labels[14:18] = 'Juno flyby'
      labels[19:20] = 'EW10 S'
      labels[21:22] = 'EW20 S'
      labels[23:27] = 'NS on disk'
      labels[28:29] = 'NS10 W'
      labels[30:31] = 'NS20 W'
      labels[32:33] = 'NS10 E'
      labels[34:35] = 'NS20 E'
      labels[36:38] = 'NS on disk'
      labels[39:41] = 'EW on disk'
      labels[42:44] = 'EW on disk'
      labels[45:46] = 'EW10 N'
      labels[47:48] = 'EW20 N'
      labels[49:50] = 'EW10 S'
      labels[51:52] = 'EW20 S'
      labels[53:55] = 'NS on disk'
      labels[56:57] = 'NS10 W'
      labels[58:59] = 'NS20 W'
      labels[60:61] = 'NS10 E'
      labels[62:63] = 'NS20 E'
      labels[64:66] = 'NS on disk'
      labels[67:68] = 'NS10 W'
      labels[69:70] = 'NS10 E'
      
      NaC3_range = findgen(42.- 7.) +  7. ;[7.0 , 42.0]
      GGC2_range = findgen(67.-42.) + 42  ;[43.0, 70.0]
      GGC3_range = [findgen(7), findgen(4)+67.]
      
      if filt eq 'Na'    and slit eq 'C3' then range = findgen(42.- 7.) +  7. ;[7.0 , 42.0]
      if filt eq 'gg475' and slit eq 'C2' then range = findgen(67.-42.) + 42  ;[43.0, 70.0]
      if filt eq 'gg475' and slit eq 'C3' then range = [findgen(7), findgen(4)+67.]
      
;      for i = 0, n_elements(europa_frames)-1 do begin
;        print, i, '  ', europa_frames[i], '  ', labels[i]
;      endfor
;      stop
    end
  endcase
  ...
  if part eq 0 then begin
    ...
    for i = 0, n_elements(Jupiter_frames)-1 do begin
      ...
      if filt eq 'Na' OR $
         filt eq 'gg475' and slit eq 'C3' then begin
    ...
    for file = 0, N_elements(range)-1 do begin
      i = range[file]
...
;            cgimage, europa_order_straight, minv=0, maxv=0.1
;              stop
...
;            STOP ; <--- INSPECT WL SOLUTION HERE
      ...
      MWRFITS, aperture, Dir+'\Processed\Cosmic Rays\'+europa_frames[frame]+'_'+filt+'.Cleaned.fits', header, /create
      ...
      gain = -1.0
      sigclip = 12.5
;                 la_cosmic, Dir+'\Processed\Cosmic Rays\'+europa_frames[frame]+'_'+filt+'.Cleaned.fits'     ,outsuff = "CR", sigclip = sigclip, statsec = statsec, gain = gain
      CR_result_1 = mrdfits(Dir+'\Processed\Cosmic Rays\'+europa_frames[frame]+'_'+filt+'.CleanedCR.fits'   , 0, junk_header)
      CR_Mask     = mrdfits(Dir+'\Processed\Cosmic Rays\'+europa_frames[frame]+'_'+filt+'.Cleaned-mask.fits', 0, junk_header)
...
;    stop
  ...
  if part eq 1.6 then begin
    ...
    if filt eq 'Na'    and slit eq 'C3' then sunspectrum = 7
    if filt eq 'gg475' and slit eq 'C2' then sunspectrum = 42
    if filt eq 'gg475' and slit eq 'C3' then sunspectrum = 2
    ...
    LSF_fitting_ind1  = where( abs(wl - wl[D1cen]) lt 0.25, /NULL)
    if filt eq 'Na' then LSF_fitting_ind1  = LSF_fitting_ind1[where( LSF_fitting_ind1 lt 1000., /NULL)]
    LSF_fitting_ind2  = where( abs(wl - wl[D2cen]) lt 0.25, /NULL)
      ...
      FOR i = 0, s[2] - 1 DO BEGIN
        ...
        if (labels[orientation] eq 'NS on disk') or (labels[orientation] eq 'EW on disk') $
          or (labels[orientation] eq 'Juno flyby') then begin
                      if i gt 28 and i lt 40 then begin                                                         ; HACK, i'm eyeballing how many pixels i should block out based on solar subtraction
...
;                       endif
      ...
      if filt eq 'gg475' then begin
        newimg = sunsubbed
        ;          newimg[LSF_fitting_ind1,*] = europa[LSF_fitting_ind1,*]
        ;          newimg[LSF_fitting_ind2,*] = europa[LSF_fitting_ind2,*]
      endif
...
;                  endif
        ...
        if filt eq 'gg475' then begin
          p = [max(totsubtrd[LSF_fitting_ind2]), WL[Europa_D2_pixel], 0.064]
        endif
        ...
        ;          stop
;        if a[2] gt 1. or err_a[2] gt 1.2e-4 then a = p                                                           ; some of these fits are randomly awful and my guesses are better. using my guess for these cases instead :)
        D2_Coeffs[*, i] = a
...
;                  endif
      ...
      if filt eq 'Na' then begin
        if (labels[orientation] eq 'NS on disk') or (labels[orientation] eq 'EW on disk') $
          or (labels[orientation] eq 'Juno flyby') then begin
          if D2_errors[-1] gt max(D2_errors[0:69]) then mpfitD2emission[-1] = int_D2_emission[-1]
          if D1_errors[-1] gt max(D1_errors[0:69]) then mpfitD1emission[-1] = int_D1_emission[-1]
          if D2_errors[1]  gt max(D2_errors[0:69]) then mpfitD2emission[1]  = int_D2_emission[ 1]
          if D1_errors[1]  gt max(D1_errors[0:69]) then mpfitD1emission[1]  = int_D1_emission[ 1]
        endif
      endif
...
;               endif
      ...
      column_errors = D1_errors + D2_errors
...
;      stop
      ...
      filename      = Europa_frames[orientation]+'_'+filt+'.Cleaned.fits'
      ...
      if filt eq 'Na'    then g = g_na
      if filt eq 'gg475' then g = g_K

      column                                               = area_profile * 10.e6 / g
      int_column                                           = int_profile  * 10.e6 / g
      ...
      cgplot, column/1.e10, findgen(N_elements(newimg[index0:index1, *])), /ynozero, /noerase, pos = p[*,3], ys= 5, $
        xtitle = 'K Column Density ('+cgsymbol('times')+'10!U10!N atoms / cm!U2!N)', xstyle=9;, xr=[0,400]
      ...
      cgtext, 0.63, 0.67, 'K ('+cgsymbol('times')+'10!U10!N atoms / cm!U2!N)', color='black', /normal, charsize=1.5, orientation=90.
;      cgtext, 0.66, 0.65, '('+cgsymbol('times')+'10!U10!N atoms / cm!U2!N)', color='black', /normal, charsize=1.3, orientation=90.
      ...
      ;
              ...
              cgtext, 0.24, 0.85, 'Basic redux', align = 0.5, /normal, charsize=2, color='white'
              ;   raw 1D
              cgplot, wl[index0:index1], mean(europa[index0:index1, *], dim=2, /NAN)/1.e5, pos = p[*,3], xr = xr, /xs, ytickformat = '(A1)', xtickformat = '(A1)', /noerase
      ...
      ;        cglegend, colors=['black', 'orange'], psym=[0,0], titles=['Raw spectrum', 'Fake sun spectrum'], length=0.01, symsize=0.1, /Box, Location=[0.11, 0.50], charsize=1.0, /Background, vspace=1
              cgaxis, yaxis = 0, ytitle = 'Na ('+cgsymbol('times')+'10!U6!N Rayleighs / ' + cgsymbol('Angstrom')+')', ystyle=1, charsize=1.8
      ...
      ;        cgcolorbar, POSITION=[0.105, 0.905, 0.369, 0.92], range = [min(europa[index0:index1, *])/1.e5,max(europa[index0:index1, *])/1.e5], charsize=1.8, /top, title='Na Column Density (10!U5!N Rayleighs / '+ cgsymbol('Angstrom')+')'
              ...
              ;   sun sub 1D
              cgplot, wl[index0:index1], mean(sunsubbed[index0:index1, *], dim=2, /NAN)/1.e3, pos = p[*,4], xr = xr, /xs, ytickformat = '(A1)', xtickformat = '(A1)', /noerase
              cgplot, wl[index0:index1], fltarr(index1-index0), /overplot, color='red', linestyle=2
              cgtext, 0.50, 0.85, 'Sun-subtracted', align = 0.5, /normal, charsize=2, color='white'
              cgaxis, xaxis = 0, xtitle = 'Wavelength (' + cgsymbol('Angstrom') + ')', xstyle=1, charsize=1.8
              cgcolorbar, POSITION=[0.369, 0.905, 0.630, 0.92], range = [min(mean(sunsubbed[index0:index1, *], dim=2, /NAN))/1.e3,max(mean(sunsubbed[index0:index1, *], dim=2, /NAN))/1.e3], $
                charsize=1.8, /top, title='Na ('+cgsymbol('times')+'10!U3!N Rayleighs / '+ cgsymbol('Angstrom')+')'
      
              ;   sun and io subtracted on the right hand side
              cgimage, newimg[index0:index1, *], pos = p[*,2], yr = yr, /noerase, title='Sun-Io-Sub'
              ;   sun io sub 1D
              cgplot, wl[index0:index1], mean(newimg[index0:index1, *], dim=2, /NAN)/1.e3, pos = p[*,5], ytickformat = '(A1)', xtickformat = '(A1)', /noerase, xr = xr, /xs;, yr=[min(total(eurimg, 2)), max(total(eurimg, 2))+1.e3]
              cgplot, wl[index0:index1], fltarr(index1-index0), /overplot, color='red', linestyle=2
              cgaxis, yaxis = 1, ytitle = 'Na ('+cgsymbol('times')+'10!U3!N Rayleighs / ' + cgsymbol('Angstrom')+')', ystyle=1, charsize=1.8
              cgtext, 0.77, 0.85, 'Io-subtracted', align = 0.5, /normal, charsize=2, color='white'
              cgaxis, xaxis = 0, xtitle = 'Wavelength (' + cgsymbol('Angstrom') + ')', xstyle=1, charsize=1.8
              cgcolorbar, POSITION=[0.630, 0.905, 0.895, 0.92], range = [min(mean(newimg[index0:index1, *], dim=2, /NAN))/1.e3,max(mean(newimg[index0:index1, *], dim=2, /NAN))/1.e3], $
                charsize=1.8, /top, title='Na ('+cgsymbol('times')+'10!U3!N Rayleighs / '+ cgsymbol('Angstrom')+')'
    ...
    save, /all, filename = Dir+'\Processed\'+filt+'_'+slit+'_sun_subbed_europa.sav'
    stop
  endif                               ; part 1.6


  if part eq 2 then begin                                                         ; mapping brightnesses around europa in grid structure
    RESTORE, Dir+'\Processed\'+filt+'_'+slit+'_sun_subbed_europa.sav'
    ...
    Na_EW10N_scans      = fltarr(N_elements(Nacolumns[*,0]), N_elements(Nacolumns[0,*]))
    Na_EW20N_scans      = fltarr(N_elements(Nacolumns[*,0]), N_elements(Nacolumns[0,*]))
    Na_EW10S_scans      = fltarr(N_elements(Nacolumns[*,0]), N_elements(Nacolumns[0,*]))
    Na_EW20S_scans      = fltarr(N_elements(Nacolumns[*,0]), N_elements(Nacolumns[0,*]))
    
    GG_EW10N_scans      = fltarr(N_elements(GGcolumns[*,0]), N_elements(GGcolumns[0,*]))
    GG_EW20N_scans      = fltarr(N_elements(Nacolumns[*,0]), N_elements(Nacolumns[0,*]))
    GG_EW10S_scans      = fltarr(N_elements(Nacolumns[*,0]), N_elements(Nacolumns[0,*]))
    GG_EW20S_scans      = fltarr(N_elements(Nacolumns[*,0]), N_elements(Nacolumns[0,*]))
    
    
    Na_column_errors    = fltarr(N_elements(Nacolumns[*,0]), N_elements(Nacolumns[0,*]))
    GG_column_errors    = fltarr(N_elements(GGcolumns[*,0]), N_elements(GGcolumns[0,*]))
    ...
    ;  THIRD:  Na linewidth map
      ...
      no_magiq                          = fltarr(s[1], s[2], n_elements(europa_frames))
      dummy_cube                        = fltarr(s[1], s[2], n_elements(guiders))
      keepindex                         = []
      
      
      
      FOR orientation = 0, N_elements(range)-1 DO BEGIN
        file = range[orientation]
...
;           filt eq 'gg475' and (labels[file] eq 'EW on disk') then continue                                                   ; unfortunately all the on disk K measurements are unsalvageable #SkillIssue:/
        ...
        slit_loc                          = fltarr(NSshort[1]-NSshort[0]+1., NSlongs[1]-NSlongs[0]+1.)
        slitsize                          = size(slit_loc)
...
;        if filt eq 'Na'    then column = int_Nacolumns[*, file]
;        if filt eq 'gg475' then column = int_GGcolumns[*, file]
        
        
        if run eq 0      AND filt eq 'Na'                                 then begin
          column = int_Nacolumns[*, file]
        endif
        
        if run eq 1      AND filt eq 'Na'                                 then begin
          column       = int_Nacolumns[*, file]
          
          if labels[file] eq 'EW10 N'                                     then begin
            Na_EW10N_scans[*, file]  = column                                                                            ; this is how i will save EW10 N scans of Na and K to get their ratio
            Na_column_errors[*,file] = column_errors
          endif
          
          if labels[file] eq 'EW20 N'                                     then begin
            Na_EW20N_scans[*, file]  = column                                                                            ; this is how i will save EW20 N scans of Na and K to get their ratio
            Na_column_errors[*,file] = column_errors
          endif
          
          if labels[file] eq 'EW10 S'                                     then begin
            Na_EW10S_scans[*, file]  = column                                                                            ; this is how i will save EW10 S scans of Na and K to get their ratio
            Na_column_errors[*,file] = column_errors
          endif
          
          if labels[file] eq 'EW20 S'                                     then begin
            Na_EW20S_scans[*, file]  = column                                                                            ; this is how i will save EW20 S scans of Na and K to get their ratio
            Na_column_errors[*,file] = column_errors
          endif
        ...
        if run eq 2      AND filt eq 'Na'                                 then begin
          column       = Na_linewidths[*, file]
        endif
        
        
        
        if run eq 0      AND filt eq 'gg475'                              then begin
          column                     = GGcolumns[*, file]
        endif
        
        if run eq 1      AND filt eq 'gg475'                              then begin
          column       = int_GGcolumns[*, file]
          
          if labels[file] eq 'EW10 N'                                     then begin
            GG_EW10N_scans[*, file]  = column                                                                            ; this is how i will save EW10 N scans of Na and K to get their ratio
            GG_column_errors[*,file] = column_errors
          endif

          if labels[file] eq 'EW20 N'                                     then begin
            GG_EW20N_scans[*, file]  = column                                                                            ; this is how i will save EW20 N scans of Na and K to get their ratio
            GG_column_errors[*,file] = column_errors
          endif

          if labels[file] eq 'EW10 S'                                     then begin
            GG_EW10S_scans[*, file]  = column                                                                            ; this is how i will save EW10 S scans of Na and K to get their ratio
            GG_column_errors[*,file] = column_errors
          endif

          if labels[file] eq 'EW20 S'                                     then begin
            GG_EW20S_scans[*, file]  = column                                                                            ; this is how i will save EW20 S scans of Na and K to get their ratio
            GG_column_errors[*,file] = column_errors
          endif
        ...
        if run eq 2      AND filt eq 'gg475'                              then begin
          column       = GG_linewidths[*, file]
        endif
        ...
        if run eq 0 or run eq 1 then slitfiller2d = SMOOTH(slitfiller2d, slitsize[1]-1, /edge_truncate, /nan)
        if run eq 2 then             slitfiller2d = SMOOTH(slitfiller2d, slitsize[1]-1, /edge_truncate, /nan)
            loadct, 3
...
;
                  if included_guiders eq !NULL then begin
                    ...
                    no_magiq[*,*,file] = mrdfits(dir+'\MAGIQ Files\hiresSlit000_0122.fits', 0, guider_header, /fscale)   ; an on disk EW frame, just for centroiding
;                    stop
                    if round(sxpar(headfits(spectra[file]), 'ROTPOSN') - 90.) eq 334 OR $
                      round(sxpar(headfits(spectra[file]), 'ROTPOSN') - 90.) eq 344 then begin          ; NS Oriented
...
;                    stop
                    ...
                    no_magiq[*,*,file] = shift(no_magiq[*,*,file], shift_in_x, shift_in_y)
                  endif
                          
        ;      layered = median(magiq_per_frame, dim=3, /even)
        layered = total(magiq_per_frame, 3) + total(no_magiq, 3)
      ...
      if filt eq 'Na'    and run eq 0 then valrange = [0.   , 3.e10]
      if filt eq 'Na'    and run eq 2 then valrange = [0.   , 0.700]
      if filt eq 'gg475' and slit eq 'C2' $
                                      then valrange = [5.e10, 3.e11]
      if filt eq 'gg475' and slit eq 'C3' $
                                      then valrange = [1.e7 ,  3.e9]
      if filt eq 'gg475' and run eq 2 and slit eq 'C2' $
                                      then valrange = [0.000, 10.00]
      ...
      radii_per_pixel = (28. / slitsize[2])/ang_radius
      
      radp,justmap,s[1]/2.,s[2]/2., radial_dist, col_dens
      radial_dist = radial_dist * radii_per_pixel
      
      
      
      bin_size = 3.
      window, 7
      cghistoplot, radial_dist, binsize=bin_size, REVERSE_INDICES = ri, LOCATIONS = r_array, MININPUT = 0
      i_array = fltarr(N_elements(r_array))
      errors  = i_array
      for i = 0, N_elements(r_array)-1 do begin
        i_array[i] = mean(col_dens[ri[ri[i]:ri[i+1]-1]], /nan)
        errors[i]  = stddev(col_dens[ri[ri[i]:ri[i+1]-1]], /nan)
      endfor
      ;truncate at 85,000 km -- few few real pixels left out there
;      r_array = r_array[0:85] & i_array = i_array[0:85]

      window, 7
      cgplot, radial_dist, col_dens, xr=[0,33]
      cgplot, r_array, i_array, /overplot, color='red'
      
      
      col_ondisk = max(i_array)
      maybe_here = []
      
      for k = 0, N_elements(r_array)-1 do begin
        drop_loc = col_ondisk / 2.718281828
        maybe_here = [maybe_here, where(i_array[k] gt drop_loc-errors[k] and i_array[k] lt drop_loc+errors[k])] 
      endfor  
      ...
      if run eq 0 then begin
        cgPS_Open, filename = dir+'\Figures\'+filt+'_'+slit+'_guider_map_Juno.eps', /ENCAPSULATED, xsize = 10, ysize = 10
        !P.font=1
        ...
        cgcolorbar, POSITION=[0.7, 0.35, 0.72, 0.65], range = [min(justmap), max(justmap)]/1.e10, /vertical, /right, color='white', $
          title=filt+' (x10!U10!N cm!U-2!N)', charsize=2.5
...
;        cgaxis, 0.4, 0.2, color='white', /data, xaxis=0, xr = yr
      ...
      if run eq 1 then begin
        cgPS_Open, filename = dir+'\Figures\'+filt+'_'+slit+'_guider_map_Juno_INTEGRATED.eps', $
...
;        cgaxis, 250., 40., color='white', /data, xaxis=0, xr = yr
        ...
        cgPS_Close
        if filt eq 'gg475' and slit eq 'C3' then begin                                                ; using integrated map for the ratios FOR NOW
          K_map = justmap
          save, K_map , GGcolumns, GG_EW10N_scans, GG_EW20N_scans, GG_EW10S_scans, GG_EW20S_scans, filename = Dir+'\Processed\'+filt+'_'+slit+'_map.sav'
        endif
        
        if filt eq 'Na'    and slit eq 'C3' then begin
          Na_map = justmap
          save, Na_map, Nacolumns, Na_EW10N_scans, Na_EW20N_scans, Na_EW10S_scans, Na_EW20S_scans, filename = Dir+'\Processed\'+filt+'_'+slit+'_map.sav'
        endif
        
      endif
      
      if run eq 2 then begin
        cgPS_Open, filename = dir+'\Figures\'+filt+'_'+slit+'_linewidth_map.eps', $
          /ENCAPSULATED, xsize = 10, ysize = 10
        !P.font=1
        if filt eq 'Na' then loadct, 3
        if filt eq 'gg475' then loadct, 1
        ...
        cgimage, justmap, minv=valrange[0], maxv=valrange[1]
        cgcolorbar, POSITION=[0.7, 0.35, 0.72, 0.65], range = [min(justmap), max(justmap)], /vertical, /right, color='white', $
          title=filt+' FWHM ('+cgsymbol('Angstrom')+')', charsize=2.5
;        cgaxis, 40., 100., color='white', /data, yaxis=0, yr = yr
;        cgaxis, 250., 40., color='white', /data, xaxis=0, xr = yr
    ...
    if part eq 3 then begin
;; need to get Na/K ratios

      
      RESTORE, Dir+'\Processed\Na_C3_map.sav'
      RESTORE, Dir+'\Processed\gg475_C3_map.sav'
      
      
      s = size(Na_EW10N_scans)
      
      
      scans  = fltarr(s[1], s[2], 8)
      scan_meds = fltarr(s[1], 8)
      
      scans[*,*,0] =  Na_EW10N_scans
      scans[*,*,1] =  Na_EW20N_scans
      scans[*,*,2] =  Na_EW10S_scans
      scans[*,*,3] =  Na_EW20S_scans
      
      scans[*,*,4] =  GG_EW10N_scans
      scans[*,*,5] =  GG_EW20N_scans
      scans[*,*,6] =  GG_EW10S_scans
      scans[*,*,7] =  GG_EW20S_scans
      
      
      for scan = 0, N_elements(scans[0,0,*])-1 do begin
        window, 0
        cgimage, scans[*,*,scan]
        keepindex = []
        for i = 0, s[2]-1 do begin
          if total(scans[*,i,scan]) eq 0. then continue
          keepindex = [keepindex, i]
        endfor
        keep_size = size(keepindex)
        if keep_size[1] gt 1 then scan_med = median(scans[*,keepindex,scan], dim=2) 
        if keep_size[1] eq 1 then scan_med =        scans[*,keepindex,scan]
        
        scan_meds[*,scan] = scan_med
        
      endfor  
      
      
      print, 'EW 10 N has an Na/K ratio of ', median(scan_meds[*,0]) / median(scan_meds[*,4])
      print, 'EW 20 N has an Na/K ratio of ', median(scan_meds[*,1]) / median(scan_meds[*,5])
      print, 'EW 10 S has an Na/K ratio of ', median(scan_meds[*,2]) / median(scan_meds[*,6])
      print, 'EW 20 S has an Na/K ratio of ', median(scan_meds[*,3]) / median(scan_meds[*,7])
      
      EW10N_ratio = median(scan_meds[*,0]) / median(scan_meds[*,4])
      EW20N_ratio = median(scan_meds[*,1]) / median(scan_meds[*,5])
      EW10S_ratio = median(scan_meds[*,2]) / median(scan_meds[*,6])
      EW20S_ratio = median(scan_meds[*,3]) / median(scan_meds[*,7])
      
      ratios = [EW10N_ratio, EW20N_ratio, EW10S_ratio, EW20S_ratio]
      ...
      window, 0
      cgplot, max(Na_EW10N_scans, dim=2), color='blue'
      cgplot, max(GG_EW10N_scans, dim=2), /overplot, color='red'
      
      window, 2, title='Na/K EW10N'
      cgplot, max(Na_EW10N_scans, dim=2) / max(GG_EW10N_scans, dim=2) 
...
;      cgimage, Na_map / K_map, minv=.5, maxv=.8
