...
PRO Keck_Europa_Flyby_K, part = part, dir = dir, filt = filt

  case filt of
    'Na': begin
      ;Europa           = '1003517' ; aka C/2017 K2 (PANSTARRS)
      biases          = string(indgen(5)+55, format='(I4.4)')
  ...
  if part eq 0 then begin
    hires_x_pixels = 2139
;    if filt eq 'Na' then hires_x_pixels = 2139
;    if filt eq 'gg475' then hires_x_pixels = 3210

    hires_y_pixels = 4096
    big_array = fltarr(hires_x_pixels, hires_y_pixels, N_elements(biases))
    ...
    cgimage, bytscl(rotate(transpose(reform(bias)),7))
    ;
    ;    tester = big_Array[*,*,2]
    ;    stop

    big_array = fltarr(hires_x_pixels, hires_y_pixels, N_elements(flats))
    ...
    big_array = fltarr(hires_x_pixels, hires_y_pixels, N_elements(lamps))
    ...
    for i = 2, n_elements(Europa_frames)-1 do begin
      ...
      ;
      ;    window, 0
      ;;    cgimage, test[*,0:713], minv=900, maxv=1000
  ...
  if part eq 1 then begin ; Straighten, extract and get the wavelength solutions
    ...
    K_cube          = fltarr(4001, 44, N_elements(Europa_frames))                                 ; the number 26 is based on the aperture upper and lower bounds
    ...
    K_Jupiter_cube  = fltarr(4001, 44, N_elements(Jupiter_frames))
    ...
    for h = 0, N_elements(orders) - 1 do begin
      ...
      if filt eq 'gg475' then begin
        labels = strarr(69)
          labels[38:40] = 'EW on disk'
          labels[41:42] = 'EW10 N'
          labels[43:44] = 'EW20 N'
          labels[45:46] = 'EW10 S'
          labels[47:48] = 'EW20 S'
          labels[49:51] = 'NS on disk'                     ; file 147 is skipped because it's the sunlight spectrum
          labels[52:53] = 'NS10 W'
          labels[54:55] = 'NS20 W'
          labels[56:57] = 'NS10 E'
          labels[58:59] = 'NS20 E'
          labels[60:62] = 'NS on disk'
          labels[63:64] = 'NS10 W'
          labels[65:66] = 'NS10 E'
          labels[67:68] = 'NS on disk'
      endif

      for frame = 38, N_elements(Europa_frames)-1 do begin ; files 2-37 are all of the Na filtered spectra! ;26 for worst cr frame
...
;                STOP ; <--- INSPECT WL SOLUTION HERE
  ...
  if part eq 1.5 then begin ; ==================== FLUX CALIBRATE ALL ORDERS INTO RAYLEIGHS PER ANGSTROM UNITS ===============================
    ...
    FOR order = 0,1 do begin; N_elements(orders) - 1 do begin
      ...
      for f = 0, N_elements(K_cube[0,0,*])-1 do begin
  ...
  if part eq 1.6 then begin
    ...
    for order_index = 2,2 do begin
      ...
      sunspectrum = 4
      ondisk = (REFORM(cube[*,*,sunspectrum])) ;  0.5*(REFORM(cube[*,*,20]))                            ; took a random one, this is EW on disk
      ...
      sunrow    = ondisk[*, suncol]
      ;      cgplot, sunrow[fitindices]


      guess_peaks = []
      all_EW      = []
      FOR orientation = 3, N_Elements(cube[0,0,*])-1 DO BEGIN ;cube[0,0,*]) - 1 DO BEGIN       ; N_Elements(orientations[0,0,*]) - 1 DO BEGIN
        ...
        FOR i = 0, s[2] - 1 DO BEGIN
          ...
          fa = {x:sunrow[fitindices], y:row[fitindices], err:sqrt(abs(row[fitindices]))}
          ...
          if (labels[orientation] eq 'NS on disk') or (labels[orientation] eq 'EW on disk') $
            ...
            if i gt 16 and i lt 28 then begin
              ;                if qualitymetric lt 0 or qualitymetric gt 0.001 then begin
        ...
        cgimage, sunsubbed[fitindices,*]

        ;
        ;        loadct, 3
        ;        window, 7
        ;        cgimage, dummy_europa[500:1000,*]
        ;        window, 8
        ;        cgimage, dummy_sunspec[500:1000,*]
        ;
        ;        sunscan     = mean(dummy_europa[500:1000,*], dim=1, /nan)
        ;        sunmean     = max(mean(dummy_sunspec[500:1000,*], dim=1, /nan))
        ;                  window, 1
        ;                  cgplot, sunscan
        ;                  cgplot, mean(dummy_sunspec[500:1000,*], dim=1, /nan), /overplot, color='red'
        ;stop
        ;        sunratio = sunscan / sunmean
        ;                  cgplot, sunratio
        ;
        ;        sunratio2d = REBIN(TRANSPOSE(sunratio), s[1], s[2])
        ;
        ;        fakesun = sunratio2d * sunspec
        ;        window, 2, title=labels[orientation]+' raw'
        ;        cgimage, europa[500:1000,*]
        ;
        ;        fakesun_spec = total(fakesun[500:1000,*], 2, /nan)
        ;
        ;        window, 3
        ;        cgplot, total(europa[500:1000,*], 2, /nan)
        ;        cgplot, fakesun_spec, /overplot, color='orange'
        ;stop
        ;        sunsubbed = europa - fakesun
        ...
        ;              if i gt 16 and i lt 28 then begin
        ;;                if qualitymetric lt 0 or qualitymetric gt 0.001 then begin
        ;                  row = !values.F_nan                                                              ; sets threshold and gets rid of sunlight over disk
        ;
        ;;                endif
        ;              endif
        ;          endif
        ;          sunsubbed[*,i] = row
        ;;          scalesubb[i,*] = scaled_totsubtrd
        ;
        ;        ENDFOR ; row by row
        loadct, 3
        window, 0, title=labels[orientation]+' sun subtracted'
        ...
        for i = 0, s[2] - 1 DO BEGIN
          ...
          cgplot, WL[index0:index1], no_io[index0:index1], /overplot, color='pink'                                ; pink  = black (sun-subbed, NOT spatially resolved) row - fake io row
        ...
        cgimage, newimg[500:1000,*]

        window, 1, title=labels[orientation]+' Io subtracted'
        cgplot, wl[index0:index1], total(newimg[500:1000,*], 2, /nan)
        ...
        FOR i = 0, s[2]-1 DO BEGIN
          ...
          if strmid(labels[orientation],2,1) eq '1' then $
            p = [abs(guess_peak-abs(guess_low)), 5896.23, 0.065, abs(median(totsubtrd[index0:index1]))]
          if strmid(labels[orientation],2,1) eq '2' then $
            p = [abs(guess_peak-abs(guess_low)), 5896.23, 0.080, abs(median(totsubtrd[index0:index1]))]
          ...
          D1fa                  = { x:wl[LSF_fitting_ind1], y:y1, err:100.*sqrt(abs(y1)) }
          ...
          if labels[orientation] eq 'EW on disk' or $
            ...
            p                     = [abs(guess_peak-abs(guess_low)), 5896.25, 0.065, abs(median(totsubtrd[index0:index1]))]
            parinfo               = replicate({value:0., fixed:0, limited:[0,0], limits:[0.,0.]}, n_elements(p))
            parinfo.value         = p
            parinfo[0].limited    = [1, 1]                                                              ;
            parinfo[0].limits     = [abs(guess_low), guess_peak]                                        ;
          ...
          ; Inspect:
          ;                          window, 2, xs=800, ys=500, title=labels[orientation]+' D1 '+strcompress(i)
          ...
          if strmid(labels[orientation],2,1) eq '1' then $
            p = [abs(guess_peak-abs(guess_low)), 5890.25, 0.068, abs(median(totsubtrd[index0:index1]))]
          if strmid(labels[orientation],2,1) eq '2' then $
            p = [abs(guess_peak-abs(guess_low)), 5890.25, 0.078, abs(median(totsubtrd[index0:index1]))]
          ...
          parinfo[0].limits     = [abs(guess_low), guess_peak]                                             ;
          parinfo[1].limited    = [1, 1]                                                              ;
          parinfo[1].limits     = [wl[LSF_fitting_ind2[loc]]-0.1, wl[LSF_fitting_ind2[loc]]+0.1]                                                  ;
          ;          parinfo[1].fixed      = 1                                                                   ;
          ...
          D2fa                  = { x:wl[LSF_fitting_ind2], y:y2, err:100.*sqrt(abs(y2)) }
          ...
          if labels[orientation] eq 'EW on disk' or $
            ...
            p                     = [abs(guess_peak-abs(guess_low)), 5890.25, 0.06, abs(median(totsubtrd[index0:index1]))]
            parinfo               = replicate({value:0., fixed:0, limited:[0,0], limits:[0.,0.]}, n_elements(p))
            parinfo.value         = p
            parinfo[0].limited    = [1, 1]                                                              ;
            parinfo[0].limits     = [abs(guess_low), guess_peak]
        ...
        ;        cgplot, mpfitD1emission, /ynozero, title='total fits over all rows'       ; units?
        ;        cgplot, int_D1_emission, /overplot, color='red'                           ; units [R]
        ;
