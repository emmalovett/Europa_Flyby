  ...
  if part eq 1.6 then begin
    ...
    for order_index = 2,2 do begin
      ...
      if filt eq 'Na' then begin
        EW0__   = 0.5*(REFORM(MEDIAN(cube[*,*,2:4]  , dim=3)));+cube[*,*,34:36],dim=3)))                                        ; EW orientation on-disk
        EW10_N  = 0.5*(REFORM(MEDIAN(cube[*,*,5:6]  , dim=3)))
        EW20_N  = 0.5*(REFORM(MEDIAN(cube[*,*,7:8]  , dim=3)))
        Juno    = 0.5*(REFORM(MEDIAN(cube[*,*,9:13] , dim=3)))
        EW10_S  = 0.5*(REFORM(MEDIAN(cube[*,*,14:15], dim=3)))
        EW20_S  = 0.5*(REFORM(MEDIAN(cube[*,*,16:17], dim=3)))
        NS0__   = 0.5*(REFORM(MEDIAN(cube[*,*,18:22], dim=3)));+cube[*,*,31:33], dim=3)))                                     ; NS orientation on-disk
        NS10_W  = 0.5*(REFORM(MEDIAN(cube[*,*,23:24], dim=3)))
        NS20_W  = 0.5*(REFORM(MEDIAN(cube[*,*,25:26], dim=3)))
        NS10_E  = 0.5*(REFORM(MEDIAN(cube[*,*,27:28], dim=3)))
        NS20_E  = 0.5*(REFORM(MEDIAN(cube[*,*,29:30], dim=3)))
        
;        EW0__   = 0.5*(REFORM(cube[*,*,2]));:4]+cube[*,*,34:36],3)))
;        Juno    = 0.5*(REFORM(cube[*,*,9]));:13] , 3)))
;        NS0__   = 0.5*(REFORM(cube[*,*,18]));:22]+cube[*,*,31:33], 3)))
      endif
...
;      labels = ['NS0__','EW0__','JUNO flyby']
      ...
      FOR orientation = 0, N_Elements(orientations[0,0,*]) - 1 DO BEGIN
        ...
        if labels[orientation] eq 'NS0__' then NSprofile = profile
        if labels[orientation] eq 'EW0__' then EWprofile = profile
        ...
        if filt eq 'Na' then begin
          if orientation eq 0  then file = 19             ; hires0146
          if orientation eq 1  then file = 28             ; hires0184
          if orientation eq 2  then file = 30             ; hires0185
          if orientation eq 3  then file = 24             ; hires0180
          if orientation eq 4  then file = 26             ; hires0181
          if orientation eq 5  then file = 3              ; hires0130
          if orientation eq 6  then file = 6              ; hires0133
          if orientation eq 7  then file = 8              ; hires0135
          if orientation eq 8  then file = 15             ; hires0172
          if orientation eq 9  then file = 17             ; hires0174
          if orientation eq 10 then file = 10             ; hires0137
        endif
        ...
        cgplot, column/1.e9, findgen(N_elements(newimg[index0:index1, *])), /ynozero, /noerase, pos = p[*,3], ys= 5, $
        ...
        cgcolorbar, POSITION=[0.6, 0.6, 0.61, 0.9], range = [0.0,max(column/1.e9)], /vertical, charsize=1, /right
  ...
  if part eq 2 then begin                                                         ; mapping brightnesses around europa in grid structure
    ...
    if filt eq 'Na' then begin
      ...
      FOR file = 3, 37-1 DO BEGIN                                                             ; these are the Na filter files
        ...
        slit_location_Juno                = ROT(junoslit_shift, 316, 1, (NSshort[1]+NSshort[0])/2., (NSlongs[1]+NSlongs[0])/2.+15.)                  ; HACK
        slit_location_344                 = ROT(NSslit_locations, 350)
        ...
        cgcolorbar, POSITION=[0.7, 0.35, 0.72, 0.65], range = [min(layered), max(layered)]/10000000., /vertical, /right, color='white';, Format='(F0.2)'
