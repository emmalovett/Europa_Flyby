...
FUNCTION match_scattered_sunlight, p, x=x, y=y, err=err, fit=fit ; use with MPFIT
  ...
  ;fit = P[0]*GAUSS_SMOOTH(x,P[2],/EDGE_TRUNCATE) + P[1] + P[3]*fitindices ; 4 coeffs with slope
...
FUNCTION scale_fit_sunlight, p, x
  common sunlight_fit_common, fitindices
  ;return, P[0]*GAUSS_SMOOTH(x,P[2],/EDGE_TRUNCATE) + P[1] + P[3]*findgen(N_elements(x))
...
PRO Keck_Europa_Flyby, part = part, dir = dir, filt = filt
  ...
  case dir of
    'Z:\DATA\Keck\Europa Na\HIRES_20220928': begin
      ...
      Star_Frames     = string(129,            format='(I4.4)')                                   ; use a europa frame bc we have no star frame
...
;      Europa_array[*,*,i] = Europa_array[*,*,i] / flat & PRINT, 'Do not apply this flat, it needs spectral *and* spatial normalization to unity'
  ...
  order_60 = {guess_coeffs:[5873.10,0.0285014,-5.54225e-007], low_bound:448,  hi_bound:501,  WL_range:[5576., 5578], aperture_limit:[0,44], name:'order_60'}              ; Na order
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; STRAIGHTEN, EXTRACT AND GET THE WAVELENGTH SOLUTIONS ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

  if part eq 1 then begin

    if filt eq 'Na'    then begin
      slit_length_pix = 78.
      N_files         = 35.                                                                                           ; rough width of the aperture in pixels and # of Na filtered Europa frames
    endif
    ...
    Na_cube         = fltarr(4001, slit_length_pix, N_files) 
    K_cube          = fltarr(4001, slit_length_pix, N_files) 
    O_cube          = fltarr(4001, slit_length_pix, N_files)                                                     
...
;      xr          = minmax(WL)
      ...
      if filt eq 'Na' then begin
        ...
        if filt eq 'Na' then begin
          ...
          labels[12:13] = 'EW10 S'
          labels[14:15] = 'EW20 S'
          labels[16:20] = 'NS on disk'                     ; file 147 is skipped because it's the sunlight spectrum
          labels[21:22] = 'NS10 W'
          labels[23:24] = 'NS20 W'
          labels[25:26] = 'NS10 E'
          labels[27:28] = 'NS20 E'
      ...
      statuses = []
      for frame = 0, N_files-1 do begin ; files 2-37 are all of the Na filtered spectra! ;26 for worst cr frame
...
;        STOP ; <--- INSPECT WL SOLUTION HERE
      ...
      save, Na_cube, K_cube, O_cube, Na_Jupiter_cube, K_Jupiter_cube, O_Jupiter_cube, Na_WL, K_WL, O_WL, labels, filename = Dir+'\Processed\.sav'
...
;stop
      ...
      save, Na_cube, K_cube, O_cube, Na_Jupiter_cube, K_Jupiter_cube, O_Jupiter_cube, Na_WL, K_WL, O_WL, labels, ang_radius, _2D_sensitivity, filename = Dir+'\Processed\_Flux_Calibrated.sav'
...
; going to take an on-disk spectrum, get the brightest row and say that's sunlight. then i will rebin that one row into a 2D image and use that as my sunlight spectrum
      ...
      ondisk     = REFORM(cube[*,*,sunspectrum]) ;  0.5*(REFORM(cube[*,*,20]))   
      ...
      guess_peaks = []
      all_EW      = []
      FOR orientation = 0, N_Elements(cube[0,0,*])-1 DO BEGIN ;cube[0,0,*]) - 1 DO BEGIN       ; N_Elements(orientations[0,0,*]) - 1 DO BEGIN
        ...
        FOR i = 0, s[2] - 1 DO BEGIN
          ...
          row[WHERE(row lt -1.e3, /null)] = 0.
          
          guess_scale = median(row[fitindices] / sunrow[fitindices]) ; Guess multiplicative coeff.
          
          ; there's some tilt in the sunlight spectrum relative to the individual frames, try to fit it with a 1st order poly
            ;fit_this = row[fitindices] - guess_scale*sunrow[fitindices]
            ;window, 3
            ;cgplot, fitindices, fit_this, psym = 1
            ;coeffs = poly_fit(fitindices, fit_this, 1)
            ;cgplot, fitindices, poly(fitindices, coeffs), /overplot
          
          ; Fit a y = A*Gauss_smooth(x,C) + B + D*index function to the spectrum, where x is the reference solar spectrum
            ;p0 = double([guess_scale, coeffs[0], 0.01, coeffs[1]])           ; Guess at the 3 initial coefficients
...
;            parinfo[0].limits     = [1./5.d4, 1.d1]     ; multiplicative scaling limits, never negative
            parinfo[0].limits     = [p0[0]*0.5,p0[0]*2.]
            parinfo[1].limited    = [1, 1]
            parinfo[1].limits     = [-2.d3, 2.d3]   ; additive offset limits
            parinfo[2].limited    = [1, 1]
            parinfo[2].limits     = [0.d, 10.d]      ; smoothing limits, never negative 
            
          fa = {x:double(sunrow[fitindices]), y:double(row[fitindices]), err:double(make_array(s[1], value=stddev(row[fitindices])))}
          p = mpfit('match_scattered_sunlight', p0, PERROR = err_P, functargs=fa, status=status, $
                    parinfo=parinfo, quiet=quiet, xtol=1.d-12, ftol=1.d-12, gtol=1.d-15)
          P_guessed[*,i]  = p0
          p_returned[*,i] = [p, status]
          guessed_sunlight = scale_fit_sunlight(p0, sunrow) 
...
;                window, 2, title='row'+i;, xs=800, ys=500 ; all plots in DN/s
;                cgplot, wl[fitindices], row[fitindices], color = 'green', psym=16, xr = [5887., 5901.]
...
; ---------------------------------------------------- Na D1 line --------------------------------------------------------
        ...
        FOR i = 0, s[2]-1 DO BEGIN
          ...
          yfit            = mpfitpeak(D1fa.x, D1fa.y, a, nterms=3, ESTIMATES = p, PERROR = err_a)
          D1_Coeffs[*, i] = a
